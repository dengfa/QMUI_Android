...
import com.qmuiteam.qmui.shadow.ShapeConst.TOP_LEFT
import com.qmuiteam.qmui.shadow.ShapeConst.TOP_RIGHT
...
class ShapeButton @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private val shadowRoundRectDrawable: ShadowRoundRectDrawable by lazy(LazyThreadSafetyMode.NONE) {
        ShadowRoundRectDrawable(Params(floatArrayOf()))
    }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        if (changed) {
            touchRect?.apply {
                this.left = left - touchAddLeft
                this.top = top - touchAddTop
                this.right = right + touchAddRight
                this.bottom = bottom + touchAddBottom
    ...
    public fun updateBgColor(startColor: Int, endColor: Int) {
        ...
        modifyAttribute(startColor = startColor, endColor = endColor)
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when {
            ...
            shadowBlurRadius > 0 && !activeEnable -> {
                canvasSave = true
                canvas.save()
                val verticalOffset = (-(paddingTop - shadowRoundRectDrawable.desiredTopPadding) + paddingBottom - (shadowRoundRectDrawable.desiredBottomPadding)) / 2
                val horizontalOffset = (-(paddingLeft - shadowRoundRectDrawable.desiredLeftPadding) + (paddingRight - shadowRoundRectDrawable.desiredRightPadding)) / 2
