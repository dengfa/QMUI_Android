...
package com.qmuiteam.qmui.behavior;
...
/**
 * 在BottomSheetBehavior基础上修改较大，只适用于学习首页的交互
 */
public class VpBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {

    public static final float DAMPING = 0.5f;//阻尼值
    ...
    @IntDef({
        STATE_EXPANDED, STATE_COLLAPSED, STATE_DRAGGING, STATE_SETTLING, STATE_HIDDEN, STATE_HALF_EXPANDED
    })
    ...
    public VpBottomSheetBehavior() {
    }

    public VpBottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public boolean onLayoutChild(@NonNull CoordinatorLayout parent, @NonNull V child, int layoutDirection) {
        ...
        if (viewDragHelper == null) {
            viewDragHelper = ViewDragHelper.create(parent, 0.5f, dragCallback);
        }
    ...
    public boolean onInterceptTouchEvent(@NonNull CoordinatorLayout parent, V child, @NonNull MotionEvent event) {
        ...
        View scroll = nestedScrollingChildRef != null ? nestedScrollingChildRef.get() : null;
        return action == MotionEvent.ACTION_MOVE && scroll != null && !ignoreEvents && state != STATE_DRAGGING && !parent.isPointInChildBounds(scroll,
            (int) event.getX(), (int) event.getY()) && viewDragHelper != null && Math.abs(initialY - event.getY()) > viewDragHelper.getTouchSlop();
    }

    @Override
    public boolean onTouchEvent(@NonNull CoordinatorLayout parent, @NotNull V child, @NonNull MotionEvent event) {
    ...
    public boolean onNestedPreFling(@NonNull CoordinatorLayout coordinatorLayout,
        ...
        if (nestedScrollingChildRef != null) {
            return target == nestedScrollingChildRef.get() && (state != STATE_EXPANDED || super.onNestedPreFling(coordinatorLayout, child, target,
                velocityX, velocityY));
        } else {
            return false;
        }
    ...
    void setStateInternal(@State int state) {
        ...
        ViewCompat.setImportantForAccessibility(bottomSheet, ViewCompat.IMPORTANT_FOR_ACCESSIBILITY_YES);
    ...
    View findScrollingChild(View view) {
        ...
        if (view instanceof ViewPager) {
            //解决嵌套Viewpager时的滑动问题
            View currentViewPagerChild = ViewPagerUtils.getCurrentView((ViewPager) (view)); //通过ViewPagerUtils找到当前在界面上的页面
        ...
        if (view instanceof ViewGroup) {
            ...
            while (i < count) {
                View scrollingChild = findScrollingChild(((ViewGroup) view).getChildAt(i));
                if (scrollingChild != null) {
                    return scrollingChild;
                }
                i++;
    ...
    private final ViewDragHelper.Callback dragCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(@NonNull View child, int pointerId) {
            ...
            if (state == STATE_EXPANDED && activePointerId == pointerId) {
                final View scroll = nestedScrollingChildRef != null ? nestedScrollingChildRef.get() : null;
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
            ...
            startSettlingAnimation(releasedChild, targetState);
        }

        @Override
        public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
            final int max = hideable ? parentHeight : collapsedOffset;
            final int min = getExpandedOffset();
            if (top < min) {
                return min;
            } else if (top > max) {
                return max;
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && callback != null) {
            if (top > collapsedOffset) {
                callback.onSlide(bottomSheet, (float) (collapsedOffset - top) / (parentHeight - collapsedOffset));
            } else {
                callback.onSlide(bottomSheet, (float) (collapsedOffset - top) / (collapsedOffset - getExpandedOffset()));
            }
        }
    }
    ...
    /**
     * A utility function to get the {@link  VpBottomSheetBehavior} associated with the {@code view}.
     *
     * @param view The {@link View} with {@link  VpBottomSheetBehavior}.
     * @return The {@link  VpBottomSheetBehavior} associated with the {@code view}.
     */
    @SuppressWarnings("unchecked")
    public static <V extends View> VpBottomSheetBehavior<V> from(V view) {
        ViewGroup.LayoutParams params = view.getLayoutParams();
        if (!(params instanceof CoordinatorLayout.LayoutParams)) {
            throw new IllegalArgumentException("The view is not a child of CoordinatorLayout");
        }
        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params).getBehavior();
        if (!(behavior instanceof VpBottomSheetBehavior)) {
            throw new IllegalArgumentException("The view is not associated with BottomSheetBehavior");
        }
        return (VpBottomSheetBehavior<V>) behavior;
    }
