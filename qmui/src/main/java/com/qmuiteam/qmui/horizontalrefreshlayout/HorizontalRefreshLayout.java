...
public class HorizontalRefreshLayout extends FrameLayout {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (rightEnable
                    && !canChildScrollRight()
                    && startX != 0
                    && startX - ev.getX() > touchSlop
                    && startY != 0
                    && Math.abs(startY - ev.getY()) < touchSlop / 2
                    && refreshState != REFRESH_STATE_REFRESHING
                    && rightRefreshHeader != null) {
                    //end drag
                    dragState = END;
    ...
    private void smoothRelease() {
        ...
        if (refreshMode == MODE_ABOVE) {
            ...
            mTarget.animate().translationX(0).setDuration(200).setListener(new AnimatorListenerAdapter() {
                @Override
        ...
        if (leftRefreshHeader != null) {
            leftRefreshHeader.onStart(START, leftHead);
            if (refreshMode == MODE_UNDER_FOLLOW_DRAG || refreshMode == MODE_ABOVE) {
                leftHead.animate().translationX(-leftHeadWidth).setDuration(200).start();
            }
        }
        if (rightRefreshHeader != null) {
            rightRefreshHeader.onStart(START, rightHead);
            if (refreshMode == MODE_UNDER_FOLLOW_DRAG || refreshMode == MODE_ABOVE) {
                rightHead.animate().translationX(rightHeadWidth).setDuration(200).start();
            }
        }
    }
    ...
    private void smoothLocateToRefresh(boolean manualOrAuto) {
        ...
        if (leftRefreshHeader != null && translationX > 0) {
            ...
            if (refreshMode == MODE_UNDER_FOLLOW_DRAG || refreshMode == MODE_ABOVE) {
                leftHead.animate().translationX(0).setDuration(150).start();
            }
            refreshHead = START;
        }
        if (rightRefreshHeader != null && translationX < 0) {
            ...
            if (refreshMode == MODE_UNDER_FOLLOW_DRAG || refreshMode == MODE_ABOVE) {
                rightHead.animate().translationX(0).setDuration(150).start();
            }
            refreshHead = END;
        }
        ...
        if (refreshMode == MODE_ABOVE) {
            ...
            mTarget.animate().translationXBy(dX).setDuration(manualOrAuto ? 150 : 500).setListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    setTargetTranslationX(mTarget.getTranslationX());
                    if (refreshCallback != null) {
    ...
    public void onRefreshComplete() {
        ...
        smoothRelease();
        if (leftRefreshHeader != null) {
            leftRefreshHeader.onStop(leftHead); //reset view style
        }
        if (rightRefreshHeader != null) {
            rightRefreshHeader.onStop(rightHead); //reset view style
        }
    }
    ...
    public boolean isFinish() {
        return refreshState == REFRESH_STATE_IDLE;
    }
}
