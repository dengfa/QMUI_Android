...
class CropWindowMoveHandler(
     ...
     */
    val moveType: Type, cropWindowHandler: CropWindowHandler, touchX: Float, touchY: Float
) {
    /**
    ...
    private fun moveCenter(
        ...
        rect.offset(dx, dy)
        snapEdgesToBounds(rect, bounds, snapRadius, viewWidth, viewHeight)
    }
    ...
    private fun snapEdgesToBounds(
        ...
        if (edges.left < Math.max(0f, bounds.left) + margin) {
            edges.offset(Math.max(0f, bounds.left) - edges.left, 0f)
        }
        if (edges.top < Math.max(0f, bounds.top) + margin) {
            edges.offset(0f, Math.max(0f, bounds.top) - edges.top)
        }
        if (edges.right > Math.min(bounds.right, viewWidth.toFloat()) - margin) {
            edges.offset(Math.min(bounds.right, viewWidth.toFloat()) - edges.right, 0f)
        }
        if (edges.bottom > Math.min(bounds.bottom, viewHeight.toFloat()) - margin) {
            edges.offset(0f, Math.min(bounds.bottom, viewHeight.toFloat()) - edges.bottom)
        }
    }
    ...
    private fun adjustLeft(
        ...
        if (aspectRatio > 0) {
            ...
            if (newHeight < minCropHeight) {
                newLeft = Math.max(bounds.left, rect.right - minCropHeight * aspectRatio)
                newHeight = (rect.right - newLeft) / aspectRatio
            }

            // Checks if the window is too large vertically
            if (newHeight > maxCropHeight) {
                newLeft = Math.max(bounds.left, rect.right - maxCropHeight * aspectRatio)
                newHeight = (rect.right - newLeft) / aspectRatio
            }
    ...
    private fun adjustRight(
        ...
        if (aspectRatio > 0) {
            ...
            // Checks if the window is too small vertically
            if (newHeight < minCropHeight) {
                newRight = Math.min(bounds.right, rect.left + minCropHeight * aspectRatio)
                newHeight = (newRight - rect.left) / aspectRatio
            }

            // Checks if the window is too large vertically
            if (newHeight > maxCropHeight) {
                newRight = Math.min(bounds.right, rect.left + maxCropHeight * aspectRatio)
                newHeight = (newRight - rect.left) / aspectRatio
            }
    ...
    private val minCropWidth: Float
        private get() = if (isVertical()) {
            mMinCropHeight
        } else {
            mMinCropWidth
        }
    private val maxCropWidth: Float
        private get() = if (isVertical()) {
            mMaxCropHeight
        } else {
            mMaxCropWidth
        }
    private val minCropHeight: Float
        private get() = if (isVertical()) {
            mMinCropWidth
        } else {
            mMinCropHeight
        }
    private val maxCropHeight: Float
        private get() = if (isVertical()) {
            mMaxCropWidth
        } else {
            mMaxCropHeight
    ...
    private fun adjustTop(
        ...
        if (aspectRatio > 0) {
            ...
            if (newWidth < minCropWidth) {
                newTop = Math.max(bounds.top, rect.bottom - minCropWidth / aspectRatio)
            ...
            if (newWidth > maxCropWidth) {
                newTop = Math.max(bounds.top, rect.bottom - maxCropWidth / aspectRatio)
                newWidth = (rect.bottom - newTop) * aspectRatio
            }
    ...
    private fun adjustBottom(
        ...
        if (aspectRatio > 0) {
            ...
            // Checks if the window is too small horizontally
            if (newWidth < minCropWidth) {
                newBottom = Math.min(bounds.bottom, rect.top + minCropWidth / aspectRatio)
                newWidth = (newBottom - rect.top) * aspectRatio
            }

            // Checks if the window is too large horizontally
            if (newWidth > maxCropWidth) {
                newBottom = Math.min(bounds.bottom, rect.top + maxCropWidth / aspectRatio)
                newWidth = (newBottom - rect.top) * aspectRatio
            }
